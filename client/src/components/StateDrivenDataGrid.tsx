...
const StateDrivenDataGrid = (props: IProps) => {
    ...
    const columns: GridColDef[] = [
        ...
        {field: 'actions', headerName: '', sortable: false, filterable: false, width: 100,
            renderCell: (params) => (
                <Button
                    onClick={() => {
                        const updatedRows = unfilteredRows.filter(row => row.id !== params.row.id);
    ...
    const applySearchValueFilter = (rows: DemoRow[], searchValue: string): DemoRow[] => {
    ...
    const applyIsActiveFilter = (rows: DemoRow[], isActive?: boolean): DemoRow[] => {
        if (isActive === undefined) return rows;
        return rows.filter(row => row.isActive === isActive);
    }

    const applyIsDeletedFilter = (rows: DemoRow[], isDeleted?: boolean): DemoRow[] => {
